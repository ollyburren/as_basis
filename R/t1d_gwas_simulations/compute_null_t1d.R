...
library(snpStats)
## get the data
#args<-list(support_file='/home/ob219/scratch/as_basis/t1d_gwas_sim/support/22_support.RData',
...
if(FALSE){
  ...
  out.dir<-'/home/ob219/scratch/as_basis/t1d_gwas_sim/shuffle_sample_lists'
  wtccc.support<-data.table(wtccc.support)
  wtccc.support[,cohort:='WTCCC']
  t1dgc.support<-data.table(t1dgc.support)
  t1dgc.support[,cohort:='T1DGC']
  n<-10
  for(j in 1:n){
    out <- rbind(wtccc.support[,.(id,t1d=sample(t1d))],t1dgc.support[,.(id,t1d=sample(t1d))])
    write.table(out,file=file.path(out.dir,sprintf("t1d_null_%s",j)),sep="\t",quote=FALSE,row.names=FALSE)
  }
}
...
#args$sample_file <- '/home/ob219/scratch/as_basis/t1d_gwas_sim/shuffle_sample_lists/t1d_null_8'

samples<-fread(args$sample_file)
wtccc.support$t1d<-unlist(split(samples$t1d,samples$id)[wtccc.support$id])
t1dgc.support$t1d<-unlist(split(samples$t1d,samples$id)[t1dgc.support$id])
...
illumina.6 <- snp.rhs.estimates(cc ~ 1, snp.data=t1dgc.snps,data=t1dgc.support)
affy.6 <- snp.rhs.estimates(cc ~ 1, snp.data=wtccc.snps,data=wtccc.support)
illumina.6i <- snp.rhs.estimates(cc ~ 1, snp.data=t1dgc.snps,data=t1dgc.support,rules=i2a)
affy.6i <- snp.rhs.estimates(cc ~ 1, snp.data=wtccc.snps,data=wtccc.support,rules=a2i)
