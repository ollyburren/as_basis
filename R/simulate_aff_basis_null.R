...
keep<-findMissingData(aff.sims)

basis<-lapply(bases,function(b){
	b[rownames(b) != 'aff.t1d',colnames(b) %in% keep]
})

scaled.sims<-lapply(names(aff.sims),function(n){
...
names(scaled.sims)<-names(aff.sims)

pw.dir<-'/home/ob219/scratch/as_basis/ai_aff_basis_null_sims/'
...
empirical_confidence_intervals_null<-lapply(all.sims,quantile,probs=ci)
saveRDS(empirical_confidence_intervals_null,file='/home/ob219/scratch/as_basis/figure_data/analysis1_ill_basis_ci_under_null.RDS')
