...
basis.DT<-basis.DT<-get_gwas_data(m_file,ref_af_file,gwas_data_dir)
...
egpa_traits<-fread(m_file)[grep('lyons',file),]$trait

#egpa_traits <- c('jia_ERA','mpo_Pos')

## add in pso traits
egpa.DT<-get_gwas_data(m_file,ref_af_file,gwas_data_dir,TRUE,egpa_traits)
egpa.mat.emp<-create_ds_matrix(egpa.DT,shrink.DT,SHRINKAGE_METHOD)
pred.emp <- predict(pc.emp,newdata=egpa.mat.emp)
...
ggplot(biplot.DT,aes(x=PC1,y=PC2,label=glabel,ymin=PC2_ci_lo,ymax=PC2_ci_hi,xmin=PC1_ci_lo,xmax=PC1_ci_hi)) +
...
if(zDist){
  pdf("~/tmp/vasc_tree_dist_z.pdf",height=7,width=7)
  plot(hc, hang=-1,xlab="", sub="",cex=1.1,main="Z score")
  dev.off()
}else{
  pdf("~/tmp/vasc_tree.pdf",height=7,width=7)
