...
args = parse_args(opt_parser)
  ...
  shrink.file="/home/ob219/scratch/as_basis/raj_cd14_dichot_analysis/shrink.RDS"
  pc.file="/home/ob219/scratch/as_basis/raj_cd14_dichot_analysis/pc.RDS"
  gt.file="/home/ob219/scratch/as_basis/raj_cd14_dichot_analysis/gt_matrix.RDS"
  ot.file="/home/ob219/scratch/as_basis/raj_cd14_dichot_analysis/outcome_matrix.RDS"
  snps.file="/home/ob219/scratch/as_basis/raj_cd14_dichot_analysis/snps.RDS"
  ## this should probably be an argument as not sure we want to overwrite projections as this could get confusing.
  out.dir <- "/home/ob219/scratch/as_basis/raj_cd14_dichot_analysis/projections_null3/"
...
if(FALSE){
  ...
  cmd <- sprintf("Rscript --vanilla /home/ob219/git/as_basis/R/eQTL/q_raj_monocytes_dichot.R -n -i %s -o %s",range,out.dir)
  # this actual
  #cmd <- sprintf("Rscript --vanilla /home/ob219/git/as_basis/R/eQTL/q_raj_monocytes_dichot.R -i %s -o %s",range,out.dir)
  write(cmd,file="/home/ob219/git/as_basis/sh/raj_cd14_dichot_null3.txt")
}

computeORMat <- function(Y,G,n0=212,n1=210,underNull=FALSE){
...
if(FALSE){
  ...
  all.DT.null <- data.table(all.proj.null)[,c('probe','type'):=list(rownames(all.proj.null),'Permuted')]
  ...
  all.DT <- data.table(all.proj)[,c('probe','type'):=list(rownames(all.proj),'CD4+ Median')]
  ...
  library(cowplot)
  library(ggrepel)
  nplot <- ggplot(all.DT,aes(x=PC1,y=PC2)) + geom_point(alpha=0.2,size=0.7) +
  geom_point(data=data.table(pc.emp$x)[,label:=rownames(pc.emp$x)],color='firebrick',size=2) +
  geom_label_repel(data=data.table(pc.emp$x)[,label:=rownames(pc.emp$x)],aes(label=label),color='firebrick',show.legend = FALSE,size=5) +
  facet_wrap(~type)
  save_plot("~/tmp/dichot_cd4_null_vs_act_example.PDF",nplot,base_height=5)
