...
out_dir <- '/scratch/ob219/as_basis/raj_cd14_ind_analysis/'
data.dir <-  '/scratch/wallace/twas/'
load(file.path(data.dir,'raj-cd14-genotypes.RData'))
or.threshold <- 2
nsims <- 1e6
n.sample <- 2500
or.prior <- 0.01
...
options(scipen=999)
write.table(new.support,file='/scratch/ob219/as_basis/support_tab/as_basis_snp_support_cd14_eQTL.tab',col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
options(scipen=0)
keep <- sdt[sdt$pid %in% bsnps$pid,]$lid
filtered <- filtered[,keep]

## save filtered annotSnpStats objects so we have for later.
saveRDS(filtered,file.path(out_dir,'raj-cd14-basis_genotypes.RDS'))
...
saveRDS(obj,file=file.path(out_dir,'raj_cd14.RDS'))
