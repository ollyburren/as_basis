...
plots<-lapply(names(all.comparisons),function(n){
        ...
        nci<-empirical_confidence_intervals_null[[n]]
        ...
        geom_rect(xmin=0,xmax=length(levels(dat$disease)) + 0.5,ymin=nci[1],ymax=nci[3],alpha=0.1,color=NA,fill='dodgerblue') +
				geom_hline(yintercept=nci[2],color='blue')
})
