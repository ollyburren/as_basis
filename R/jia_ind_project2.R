...
in.dir <- '/home/ob219/scratch/as_basis/jia_ind_analysis/ind_basis5'
...
snp.summ <- mclapply(seq_along(by.run),function(j){
  ...
  tf <- rbindlist(lapply(fs,function(f){
    ...
    #DT[flipped==1,or:=1/or]
    #DT[flipped==0,or:=1/or]
...
jia.BT.DT <- jia.BT.DT[,c('a1','a2'):=list(allele.1,allele.2)]
#jia.BT.DT <- jia.BT.DT[flip==TRUE,c('a2','a1'):=list(allele.1,allele.2)]
jia.BT.DT <- jia.BT.DT[,.(chromosome,position,a1,a2,pid,flip)]
setnames(jia.BT.DT,'chromosome','chr')
ref.DT <- fread(file.path(support.dir,'as_basis_snps_with_af.tab'))
foo <- flip_allele(jia.BT.DT,ref.DT)
