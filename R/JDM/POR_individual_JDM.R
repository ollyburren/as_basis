 ...
 ## code to create the Posterior Odds for JDM dataset
 ...
 out_dir <- '/home/ob219/rds/hpc-work/as_basis/jdm_ind_analysis'
 # flip snps
 bsnps <- fread(ref_af_file)
 fls<-list.files(path='/home/ob219/rds/hpc-work/as_basis/JDM_basis_annotSnpStats/',pattern="*.RData",full.names=TRUE)
...
lapply(fls,function(f){
   ...
   gt.lor <- lapply(seq_along(sman$af.wrt.a2),function(j){
     if(sman$flag[j] %in% c('flip','flip_rc')){
       i <- getAFLU(round((sman$af.wrt.a2[j])*100))
   ...
   fname <- gsub("chr-([^.]+)\\.RData","chr\\1.RDS",basename(f))
