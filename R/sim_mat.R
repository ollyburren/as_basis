...
mvs.sigma.r<-function(r){
	...
	if(!is.positive.definite(r,,method="chol")){
...
## lor is log odds ratio initially compute Z score we can transform by multiplying though by 1/sqrt(2N)
sim.by.chr<-lapply(of,function(f){
	message(sprintf("Processing %s",f))
	sm<-get(load(f))
	## remove instances where maf is zero
	#idx.z<-which(sm$map$maf==0)
	#if(length(idx.z)!=0){
	#	sm$map<-sm$map[-idx.z,]
	#	sm$gt<-sm$gt[,-idx.z]
	#}
	...
	by.chr<-lapply(seq_along(ld.idx),function(i){
		...
		if(length(idx)==1){
			...
			ret<-t(rnorm(200,mean=sm$map[idx,]$lor,sd=sm$map[idx,]$se_hat))
		}else{
			gt<-sm$gt[,idx]
			colnames(gt)<-sm$map[idx,]$POS
			r<-ld(gt,gt,stats="R")
			r<-as.matrix(mvs.sigma.r(Matrix(r)))
			## for beta the covariance matrix is computed by sigma x SE * SE^T
			cov_se<-tcrossprod(se_hat)
			cov.beta<-cov_se * r
			## simulate beta
			ret<-mvs.perm(sm$map[idx,]$lor,cov.beta,n=200)
		}
		Z<- (1/se_hat) * ret
	})
...
saveRDS(all.chr.sim,file="/home/ob219/scratch/as_basis/tmp/all.chr.sim.RDS")
all.ids<-do.call('c',lapply(sim.by.chr,'[[','id'))
## check as to why this doesn't appear to work - is there a problem with our simulations - plot Z scores against true z score
library(reshape2)
## long thin
...
tmpy[,ppi:=approx.bf.z(value,maf,7982,0.5,1e-4),by=c('ld.block','variable')]
## constant variance due to sample size
var_sam<-1/sqrt(2*(3983+3999))
...
## note that the pzadj are wrong in final.t so we need to correct
...
act.basis<-createORMatrix(act,var='pZadj')
## remove aff.t1d that we want to remove
